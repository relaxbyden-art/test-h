(function(){
  ...
  // 7s per resource — without a timeout a stalled jsDelivr edge can hang the page for minutes
  var TIMEOUT_MS = 7000;
  ...
  function loadScript(src){
    return new Promise(function(resolve, reject){
      ...
      s.src = src; s.async = false; // preserves execution order for dynamically inserted scripts
  ...
  loadCss(base + "hashhedge-react.css?v=20260528");
  ...
  .then(function(){ return loadScript(base + "hashhedge-react-app-de.js?v=20260528"); })
  .catch(function(err){
    console.error("HashHedge DE loader failed", err);
    if (root && !root.childNodes.length) {
      root.innerHTML = '<div style="min-height:320px;display:flex;align-items:center;justify-content:center;background:#08080a;color:#f5f1e8;font:600 18px system-ui,sans-serif;text-align:center;padding:32px;">HashHedge konnte nicht geladen werden. Bitte aktualisiere die Seite.</div>';
    }
  });
})();
