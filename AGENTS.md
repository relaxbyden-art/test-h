...
- Active GitHub repo for new work: `https://github.com/relaxbyden-art/test-h` (`main`).
- Legacy repo `https://github.com/relaxbyden-art/hash-hedge` must stay untouched unless the user explicitly asks. Some live Tilda pages may still point to it during migration, so do not delete/rename it casually.
...
- New Tilda embeds should use:
  `https://cdn.jsdelivr.net/gh/relaxbyden-art/test-h@<SHA>/hashhedge-react-loader*.js`.
- Old published embeds may still use:
  `https://cdn.jsdelivr.net/gh/relaxbyden-art/hash-hedge@<SHA>/...`.
...
2. **Before giving any Tilda snippet, verify the SHA is reachable** on both GitHub and jsDelivr:
   - `https://raw.githubusercontent.com/relaxbyden-art/test-h/<SHA>/hashhedge-react-loader-affiliate-ru.js` must return 200 with content
   - `https://cdn.jsdelivr.net/gh/relaxbyden-art/test-h@<SHA>/hashhedge-react-loader-affiliate-ru.js` must return 200 with content
...
4. **Codex has working push** (via `git -c http.postBuffer=157286400 push https://<user>:<PAT>@github.com/relaxbyden-art/test-h.git main`). When push is needed, defer to Codex.
