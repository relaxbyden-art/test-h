...
function HashHedgeLogo({ size = 28, wordmarkColor = "var(--fg)" }) {
  const h = size;
  return (
    <a href="#" className="hh-logo" style={{ display: "inline-flex", alignItems: "center", gap: h * 0.42, textDecoration: "none" }} aria-label="HashHedge">
      {/* Diamond mark – figma shape */}
      <svg width={h * 0.86} height={h * 0.8} viewBox="0 0 24 22.4" fill="var(--accent)" style={{ flexShrink: 0, display: "block" }}>
        <path
...
function Reveal({ children, delay = "0" }) {
  return <div className={`reveal d-${delay}`} data-delay={delay}>{children}</div>;
}
  ...
  if (half) {
    return (
      <svg width={size} height={size} viewBox="0 0 16 16">
        <rect width="11" height="16" fill="#18A965" />
        <rect x="11" width="5" height="16" fill="#2a2a2a" />
        <path d="M 8 3 L 9.5 6.5 L 13 6.5 L 10.2 9 L 11.3 12.5 L 8 10.5 L 4.7 12.5 L 5.8 9 L 3 6.5 L 6.5 6.5 Z" fill="#fff" />
      </svg>
    );
  }
  return (
    <svg width={size} height={size} viewBox="0 0 16 16">
      <rect width="16" height="16" fill="#18A965" />
      <path d="M 8 3 L 9.5 6.5 L 13 6.5 L 10.2 9 L 11.3 12.5 L 8 10.5 L 4.7 12.5 L 5.8 9 L 3 6.5 L 6.5 6.5 Z" fill="#fff" />
    </svg>
  );
}

Object.assign(window, { HashHedgeLogo, Logo, useRevealOnScroll, Reveal, Counter, TPStar });
