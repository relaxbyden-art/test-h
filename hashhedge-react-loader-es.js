(function(){
  ...
  .then(function(){ return loadScript(base + "hashhedge-react-app-es.js?v=20260528"); })
  .catch(function(err){
    console.error("HashHedge ES loader failed", err);
    if (root && !root.childNodes.length) {
      root.innerHTML = '<div style="min-height:320px;display:flex;align-items:center;justify-content:center;background:#08080a;color:#f5f1e8;font:600 18px system-ui,sans-serif;text-align:center;padding:32px;">HashHedge no se pudo cargar. Actualiza la página.</div>';
    }
  });
})();
