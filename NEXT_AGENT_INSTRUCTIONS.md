...
### ПРАВИЛО №0 — синхронизация локальной папки (зафиксировано 2026-06-09)

Источник правды — **origin/main**. Все агенты работают в одной основной папке
(`/Users/bdrom/Desktop/Hash Hedge/Site`), а НЕ в копиях в /tmp.

Перед началом ЛЮБОЙ задачи:

```bash
cd "/Users/bdrom/Desktop/Hash Hedge/Site"
git fetch origin
git status            # убедись, что нет незапушенных нужных изменений!
git reset --hard origin/main
```

- `reset --hard` делать ТОЛЬКО убедившись, что всё нужное уже запушено
  (или сохранено отдельно) — иначе локальные правки будут потеряны.
- Копирование файлов из /tmp-клона обратно в папку — НЕ постоянный процесс,
  а аварийный обходной путь. После него локальная git-история отстаёт от origin
  (файлы свежие, ветка behind) — следующий агент может начать не от того состояния.
  Если пришлось так сделать — сразу после пуша синхронизируй папку (fetch + reset --hard).
- Известный затык: в `.git` может застрять `index.lock`, который из песочницы
  не удаляется (`Operation not permitted`) — тогда `commit/pull/reset` в папке падают.
  Попроси пользователя удалить файл `Site/.git/index.lock` вручную, после этого
  работай в основной папке как обычно.

### Стандартный цикл (одна правка)

1. **Синхронизируй папку с origin/main** (см. Правило №0; если git в папке заблокирован — полагайся на pin в `tilda-react-snippet-*.html` как на источник правды).
